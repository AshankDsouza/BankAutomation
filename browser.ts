...
export class BrowserSession {
    ...
    constructor(private readonly headed: boolean = false) {}
    ...
    async snapshot(): Promise<string> {
        const lines = await this.page.evaluate((refAttr) => {
    ...
    async extract(ref: string, name: string): Promise<string> {
        const { locator, selectors, description } = await this.prepare(ref);
        const text = (await locator.innerText({ timeout: 10000 })).trim();
    ...
    private async prepare(ref: string): Promise<{ locator: Locator; selectors: Selector[]; description: string }> {
        ...
        const raw = await live.evaluate((el: Element): RawCandidates => {
            const text = ((el as HTMLElement).innerText || '').replace(/\s+/g, ' ').trim();
        ...
        for (const candidate of candidates) {
            const count = await toLocator(this.page, candidate)
                .count()
                .catch(() => 0);
